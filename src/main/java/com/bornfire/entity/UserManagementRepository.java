...
public interface UserManagementRepository extends JpaRepository<UserManagementEntity, String> {
	...
	@Query(value = "select * from BIPS_MERCHANT_USER_MANAGEMENT WHERE user_id=?1 AND del_flag1='N' and ROWNUM = 1", nativeQuery = true)
	...
	@Query(value = "select * from BIPS_MERCHANT_USER_MANAGEMENT where merchant_user_id=?1 AND del_flag1='N' and ROWNUM = 1", nativeQuery = true)
	...
	@Query(value = "select * from BIPS_MERCHANT_USER_MANAGEMENT  where user_id=?1 and entry_flag='N' AND del_flag1='N' and ROWNUM = 1", nativeQuery = true)
